...
object RunningWindowProduct extends ProductReader[RunningWindowProduct] {
  override def read(in: RefMapIn, key: String, arity: Int): RunningWindowProduct = {
    require (arity == 3)
    val _in     = in.readGE()
    val _size   = in.readGE()
    val _gate   = in.readGE()
    new RunningWindowProduct(_in, _size, _gate)
  }
}
