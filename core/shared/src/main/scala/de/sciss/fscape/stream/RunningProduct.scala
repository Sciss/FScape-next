  ...
  private final class Stage[A, E <: BufElem[A]](layer: Layer)(implicit ctrl: Control, tpe: StreamType[A, E])
    ...
    def createLogic(attr: Attributes): NodeImpl[Shape] = {
      val res: RunningValueImpl[_, _] = if (tpe.isDouble) {
        new RunningValueImpl[Double, BufD](name, layer, shape.asInstanceOf[Shp[BufD]], 1.0)(_ * _)
      } else if (tpe.isInt) {
        new RunningValueImpl[Int   , BufI](name, layer, shape.asInstanceOf[Shp[BufI]], 1  )(_ * _)
      } else {
        assert (tpe.isLong)
        new RunningValueImpl[Long  , BufL](name, layer, shape.asInstanceOf[Shp[BufL]], 1L )(_ * _)
      }
