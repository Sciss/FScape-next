...
object ReverseWindow extends ProductReader[ReverseWindow] {
  override def read(in: RefMapIn, key: String, arity: Int): ReverseWindow = {
    require (arity == 3)
    val _in     = in.readGE()
    val _size   = in.readGE()
    val _clump  = in.readGE()
    new ReverseWindow(_in, _size, _clump)
  }
}
