...
object DetectLocalMax extends ProductReader[DetectLocalMax] {
  override def read(in: RefMapIn, key: String, arity: Int): DetectLocalMax = {
    require (arity == 2)
    val _in   = in.readGE()
    val _size = in.readGE()
    new DetectLocalMax(_in, _size)
  }
}
