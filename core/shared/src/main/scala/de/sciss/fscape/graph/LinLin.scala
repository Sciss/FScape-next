...
object LinLin extends ProductReader[LinLin] {
  override def read(in: RefMapIn, key: String, arity: Int): LinLin = {
    ...
    new LinLin(_in, _inLow, _inHigh, _outLow, _outHigh)
  }
}
final case class LinLin(in: GE, inLow: GE, inHigh: GE, outLow: GE, outHigh: GE) extends UGenSource.SingleOut {
  ...
  private[fscape] def makeStream(args: Vec[StreamIn])(implicit b: stream.Builder): StreamOut = {
    ...
    stream.LinLin(in = in.toDouble, inLow = inLow.toDouble, inHigh = inHigh.toDouble,
