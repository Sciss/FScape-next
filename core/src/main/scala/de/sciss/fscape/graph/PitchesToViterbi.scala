...
final case class PitchesToViterbi(lags: GE, strengths: GE,
  ...
  protected def makeUGens(implicit b: UGenGraph.Builder): UGenInLike =
    unwrap(this, Vector(lags.expand, strengths.expand, n.expand, voicingThresh.expand, silenceThresh.expand,
      octaveCost.expand, octaveJumpCost.expand, voicedUnvoicedCost.expand))

  protected def makeUGen(args: Vec[UGenIn])(implicit b: UGenGraph.Builder): UGenInLike =
    UGen.SingleOut(this, args)

  private[fscape] def makeStream(args: Vec[StreamIn])(implicit b: stream.Builder): StreamOut = {
    val Vec(lags, n, strengths, voicingThresh, silenceThresh, octaveCost, octaveJumpCost, voicedUnvoicedCost) = args
