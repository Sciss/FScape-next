...
//  * @param sync         `0` to have each channel treated independently, or `1` (default) to synchronise
//  *                     the gain across channels.
//  */
//final case class Gain(in: GE, mul: GE = 1.0, normalized: GE = 1, bipolar: GE = 1, sync: GE = 1)
//  extends UGenSource.MultiOut {
//
//  protected def makeUGens(implicit b: UGenGraph.Builder): UGenInLike =
//    unwrap(this, mul.expand +: normalized.expand +: bipolar.expand +: sync.expand +: in.expand.outputs)
//
//  protected def makeUGen(args: Vec[UGenIn])(implicit b: UGenGraph.Builder): UGenInLike = {
//    val numOutputs = args.size - 4
...
//    val mul +: normalized +: bipolar +: sync +: in = args
//    stream.Gain(in = in.map(_.toDouble), mul = mul.toDouble, normalized = normalized.toInt,
//      bipolar = bipolar.toInt, sync = sync.toInt)
//  }
//}
