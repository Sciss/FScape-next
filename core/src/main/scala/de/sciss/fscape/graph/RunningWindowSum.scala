...
/** A UGen that like `RunningSum` calculates the sum of the
  ...
  */
final case class RunningWindowSum(in: GE, size: GE, trig: GE = 0) extends UGenSource.SingleOut {
  protected def makeUGens(implicit b: UGenGraph.Builder): UGenInLike =
    unwrap(Vector(in.expand, size.expand, trig.expand))
  ...
  private[fscape] def makeStream(args: Vec[StreamIn])(implicit b: stream.Builder): StreamOut = {
    ...
    stream.RunningWindowSum(in = in.toDouble, size = size.toInt, trig = trig.toInt)
  }
}