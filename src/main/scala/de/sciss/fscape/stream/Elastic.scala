...
object Elastic {
  ...
  private final class Logic(shape: Shape)(implicit ctrl: Control)
    ...
    private[this] var bufReadIdx  = 0   // circular
    private[this] var bufWriteIdx = 0   // circular

    @inline
    private def canPushBuf  = bufReadIdx != bufWriteIdx

    @inline
    private def canPopBuf   = bufReadIdx != bufWriteIdx
    ...
    override def postStop(): Unit = {
      super.postStop()
      while (bufReadIdx < bufWriteIdx) {
        buffers(bufReadIdx).release()
        buffers(bufReadIdx) = null
        bufReadIdx = (bufReadIdx + 1) % num
      }
    }
    ...
    def process(): Unit = {
      ...
      if (shouldRead) {
        ...
        if (canPushBuf) {
          buffers(bufWriteIdx) = bufIn0
          bufIn0 = null
          bufWriteIdx = (bufWriteIdx + 1) % num
        }
        stateChange = true
      }
